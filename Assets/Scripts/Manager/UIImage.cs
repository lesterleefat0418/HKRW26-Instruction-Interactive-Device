...
{
    public static void Set(CanvasGroup _cg=null, bool _status=false, float _duration=0f, float _delay=0f, float _targetValue= 1f, TweenCallback _onComplete = null)
    {
        if (_cg != null)
        {
            _cg.DOFade(_status? _targetValue : 0f, _duration).SetDelay(_delay).OnComplete(()=> { 
                if (_onComplete != null) 
                    _onComplete.Invoke(); 
            });
